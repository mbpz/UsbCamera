...
import com.wesine.device_sdk.graphics.BitmapHelper;
import com.wesine.device_sdk.graphics.ShaderDrawable;
...
//		if (DEBUG) Log.v(TAG, "ColorPickerView:");
        ...
        mAlphaShader = new BitmapShader(BitmapHelper.makeCheckBitmap(), Shader.TileMode.REPEAT, Shader.TileMode.REPEAT);
...
//			if (DEBUG) Log.v(TAG, String.format("onLayout:(%d,%d)(%d,%d)", left, top, right, bottom));
            ...
            // 色相円用のグラデーションを初期化
            mGradientPaint.setShader(new RadialGradient(0, 0, radius, 0xffffffff, 0x00ffffff, Shader.TileMode.CLAMP));
            // スライダーの初期化
            setupAlphaRect();
            setUpValRect();
            // 選択色を更新
    ...
    private final void drawAlphaPanel(final Canvas canvas) {
        ...
        final Shader alphaShader = new LinearGradient(
                rect.left, rect.top, rect.right, rect.top, color, acolor, Shader.TileMode.CLAMP);
    ...
    private final void drawValPanel(final Canvas canvas) {
        ...
        final Shader mValShader = new LinearGradient(
                rect.left, rect.top, rect.left, rect.bottom, color, acolor, Shader.TileMode.CLAMP);
